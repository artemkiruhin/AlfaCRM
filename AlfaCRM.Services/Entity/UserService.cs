...
public class UserService : IUserService
{
    ...
    public async Task<Result<(Guid id, string username, Guid departmentId, string token)>> Login(LoginRequest request, CancellationToken ct)
    {
        try
        {
            ...
            if (user == null) return Result<(Guid id, string username, Guid departmentId, string token)>.Failure("User not found");

            var token = _jwtService.GenerateToken(user.Id);
            return Result<(Guid id, string username, Guid departmentId, string token)>.Success((user.Id, user.Username, user.DepartmentId ?? Guid.Empty, token));
        }
        catch (Exception e)
        {
            return Result<(Guid id, string username, Guid departmentId, string token)>.Failure($"Error while logging in: {e.Message}");
        }
    }
