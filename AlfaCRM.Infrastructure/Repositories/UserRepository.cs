...
{
    public async Task<UserEntity?> GetByUsernameAndPasswordAsync(string username, string passwordHash)
    {
        return await DbSet.AsNoTracking().FirstOrDefaultAsync(user => user.Username == username && user.PasswordHash == passwordHash);
    }
